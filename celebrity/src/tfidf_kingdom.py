...
def get_tfidf_features(df_data, cst=0.3, unigram=0.8, bigram=0.1, character=0.8, affixes=0.3):

    tfidf_unigram = TfidfVectorizer(ngram_range=(1, 1), sublinear_tf=True, min_df=5, max_df=0.8)
    #tfidf_bigram = TfidfVectorizer(ngram_range=(2, 2), sublinear_tf=False, min_df=20, max_df=0.5)
    character_vectorizer = CountVectorizer(analyzer='char_wb', ngram_range=(4, 4), lowercase=False, min_df=20,max_df=0.7)
